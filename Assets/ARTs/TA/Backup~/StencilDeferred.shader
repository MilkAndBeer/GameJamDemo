Shader "Hidden/Universal Render Pipeline/StencilDeferred"
{
    ...
    #include "Assets/ARTs/TA/Resources/CustomLit/LitInput.hlsl"
    #include "Assets/ARTs/TA/Resources/CustomLit/GBuffer.hlsl"
    ...
    half4 DeferredShading(Varyings input) : SV_Target
    {
        ...
        SurfaceData surfaceData = GBufferToSurface(posWS.xyz, gbuffer0, gbuffer1, gbuffer2);
        
        [branch]if (materialFlags & kMaterialFlagDefaultShading)
        {
            color = DirectDefault(unityLight, surfaceData);
        }
        [branch]if (materialFlags & kMaterialFlagHairShading)
        {
            color = DirectHair(unityLight, surfaceData);
        }
        [branch]if (materialFlags & kMaterialFlagSubsurfaceShading)
        {
            color = DirectSubsurface(unityLight, surfaceData);
        }
