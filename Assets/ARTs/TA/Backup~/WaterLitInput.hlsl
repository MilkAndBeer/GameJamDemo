...
CustomData InitializecustomData (float2 uv, float3 positionWS, float3 positionOS, float4 positionSS, float4 shadowCoord, half3 N, half mask)
{
    ...
    float foamX = GradientNoise(float2(uvX.x, uvX.y + fallFoamOffset - noiseY), _FallFoamScale);
    ...
    float foamZ = GradientNoise(float2(uvZ.x, uvZ.y + fallFoamOffset - noiseY), _FallFoamScale);
