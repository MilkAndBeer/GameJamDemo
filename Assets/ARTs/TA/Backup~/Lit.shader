Shader "Custom/Lit"
{
    Properties
    {
        [Foldout(1, 2, 0, 1)]_Mod ("Shading Mode_Foldout", float) = 1
        [KeywordEnum(Basic, Hair, Eye, Stylized)] _ShadingMode ("Mode", Float) = 0
        [Enum(UnityEngine.Rendering.BlendOp)]  _BlendOp ("BlendOp", Float) = 0
        [Enum(UnityEngine.Rendering.BlendMode)] _SrcBlend ("SrcBlend", Float) = 1
        [Enum(UnityEngine.Rendering.BlendMode)] _DstBlend ("DstBlend", Float) = 0
        [Enum(Off, 0, On, 1)]_ZWriteMode ("ZWriteMode", float) = 1
        [Enum(UnityEngine.Rendering.CullMode)]_CullMode ("CullMode", float) = 2
        [Enum(UnityEngine.Rendering.CompareFunction)]_ZTestMode ("ZTestMode", Float) = 4
        
        [Foldout(1, 2, 0, 1)]_StylizedParameter ("Stylized_Foldout", float) = 1
        [Enum(Default, 0, Metal, 1, Cloth, 2, Skin , 3)] _SurfaceType ("Surface Type", Float) = 0
        [Toggle(_FRONT)] _Front("Enable Front Light", Float) = 0
        [Toggle(_RECEIVE_SHADOWS_OFF)] _DisableShadows("Shadows Off", Float) = 0
        
        [Foldout(1, 2, 0, 1)]_HSL ("HSL_Foldout", float) = 1
        _H ("Hue", Range(0, 100)) = 0
        _S ("Saturation", Range(0, 5)) = 1
        _L ("Lightness", Range(0, 5)) = 1
        [Space(10)] 
        
        [Foldout(1, 2, 0, 1)]_BasicParameter ("Basic_Foldout", float) = 1
        _BaseColor ("Base Color", Color) = (1, 1, 1, 1)
        [NoScaleOffset] _BaseMap ("Base Map", 2D) = "white" {}
        [Toggle(_DMSMAP)] _DMSMAP ("Enable DMS Map", Float) = 0
        [NoScaleOffset] _DMSMap ("DMS Map", 2D) = "white" {}
        _Smoothness ("Smoothness", Range(0, 1)) = 0
        _Metallic ("Metallic", Range(0, 1)) = 0
        _Normal ("Normal", Range(0, 1)) = 1
        _SpecIntensity ("Specular Intensity", Range(1, 50)) = 1
        
        [Foldout(1, 2, 0, 1)]_AlphaTestParameter ("AlphaTest_Foldout", float) = 1
        [Toggle(_ALPHATEST)] _EnableAlphaTest("Enable Cutoff", Float) = 0.0
        _Cutoff ("Alpha", Range(0, 1.01)) = 0.5
        [Toggle(_DITHER)] _Dither("Enable Dither", Float) = 0
        _DitherThreshold ("Threshold", Range(0, 1)) = 1
        [Toggle(_FADE)] _Fade("Enable Fade", Float) = 0
        _FadeThreshold ("Threshold", Range(0, 1)) = 1

        [Foldout(1, 2, 0, 1)]_SubsurfaceParameter ("Subsurface_Foldout", float) = 1
        [Toggle(_SUBSURFACE)] _SubsurfaceOn ("Enable Subsurface", Float) = 0
        _SubsurfaceIntensity ("Subsurface Intensity", Range(0, 1)) = 0
        [NoScaleOffset] _SubsurfaceMap ("Subsurface Map", 2D) = "white" {}
        _SubsurfaceColor ("Subsurface Color", Color) = (1, 1, 1, 1)
        
        [Foldout(1, 2, 0, 1)]_HairParameter ("Hair_Foldout", float) = 1
        _HairColor1 ("Hair Color 1", Color) = (1, 1, 1, 1)
        _HairColor2 ("Hair Color 2", Color) = (1, 1, 1, 1)
        [NoScaleOffset] _HairSpecMask ("Hair Specular Mask Map", 2D) = "white" {}
        _SpecColor ("Hair Specular Color", Color) = (1, 1, 1, 1)
        _SpecSlide ("Hair Specular Slide", Range(0, 1)) = 1
        _Exponent ("Exponent", Range(0, 512)) = 64
        
        [Foldout(1, 2, 0, 1)] _EmissionParameter ("Emission_Foldout", float) = 1
        [Toggle(_EMISSION)] _Emission("Enable Emission", Float) = 0
        [NoScaleOffset] _EmissionMap ("Emission Map", 2D) = "white" {}
        _EmissionStrength ("Emission Intensity", Range(0, 10)) = 1
        _EmissionColor ("Emission Color", Color) = (1, 1, 1)
        
        [Foldout(1, 2, 0, 1)]_WindParameter ("Wind_Foldout", float) = 1
        [Toggle(_WIND)] _Wind("Enable Wind", Float) = 0
        _BendStrength ("Bend Strength", Range(0, 1)) = 0.5
        _BendRange ("Bend Range", Range(1, 5)) = 1
        _BendNoise ("Bend Noise", Range(0, 20)) = 5
        
        [Foldout(1, 2, 0, 1)]_SnowParameter ("Snow_Foldout", float) = 1
        [Toggle(_SNOW_OFF)] _SnowOff("Snow Off", Float) = 0
        
        [Foldout(1, 2, 0, 1)]_TriplanarParameter ("Triplanar_Foldout", float) = 1
        [Toggle(_TRIPLANAR)]_EnableTriplanar ("Enable Triplanar", Float) = 0
        _TriBlend ("Triplanar Blend", Range(0, 10)) = 3
        _UTilingXY ("U Tiling XY", Range(0, 3)) = 1
        _VTilingXY ("V Tiling XY", Range(0, 3)) = 1
        _UOffsetXY ("U Offset XY", Range(-1, 1)) = 0
        _VOffsetXY ("V Offset XY", Range(-1, 1)) = 0
        _UTilingZY ("U Tiling ZY", Range(0, 3)) = 1
        _VTilingZY ("V Tiling ZY", Range(0, 3)) = 1
        _UOffsetZY ("U Offset ZY", Range(-1, 1)) = 0
        _VOffsetZY ("V Offset ZY", Range(-1, 1)) = 0
        _UTilingXZ ("U Tiling XZ", Range(0, 3)) = 1
        _VTilingXZ ("V Tiling XZ", Range(0, 3)) = 1
        _UOffsetXZ ("U Offset XZ", Range(-1, 1)) = 0
        _VOffsetXZ ("V Offset XZ", Range(-1, 1)) = 0
        
        [Foldout(1, 2, 0, 1)]_TopParameter ("Top_Foldout", float) = 1         
        [Toggle(_TOP)] _Top("Enable Top", Float) = 0
        [NoScaleOffset] _TopBaseMap ("Top Albedo", 2D) = "white"{}
        [NoScaleOffset] _TopDMSMap ("Top DMS", 2D) = "white"{}
        _TopTiling ("Tiling", Range(0, 2)) = 1
        [Space(10)]
        
        [Foldout(1, 2, 0, 1)]_BottomParameter ("Bottom_Foldout", float) = 1         
        [Toggle(_BOTTOM)] _Bottom("Enable Bottom", Float) = 0
        [NoScaleOffset] _BottomBaseMap ("Bottom Albedo", 2D) = "white"{}
        [NoScaleOffset] _BottomDMSMap ("Bottom DMS", 2D) = "white"{}
        _BottomTiling ("Tiling", Range(0, 2)) = 1
        [Space(10)]
        
        [Foldout(1, 2, 0, 1)]_ThicknessParameter ("Thickness_Foldout", float) = 1
        [Toggle(_THICKNESS)] _Thick ("Enable Thickness", Float) = 0
        _Thickness ("Thickness", Range(0, 1)) = 0
        
        [Foldout(1, 2, 0, 1)]_HeightParameter ("Height_Foldout", float) = 1
        [Toggle(_HEIGHT)] _Height ("Enable Height", Float) = 0
        _Frequency ("Frequency", Range(0, 1)) = 1
        _Length ("Length", Range(0, 10)) = 1
        _Magnitude ("Magnitude", Range(0, 1)) = 1
        
        [Foldout(1, 2, 0, 1)]_CollisionParameter ("Collision_Foldout", float) = 1
        [Toggle(_COLLISION)] _Collision ("Enable Collision", Float) = 0
        
        [Foldout(1, 2, 0, 1)]_OtherParameter ("Others_Foldout", float) = 1
    }
    SubShader
    {
        //Forward Pass
        Pass
        {
            Name "Forward"
            Tags 
            {
                "LightMode" = "UniversalForward"
            }
            
            // Render State Commands ---------------
            BlendOp [_BlendOp]
            Blend [_SrcBlend] [_DstBlend]
            ZWrite [_ZWriteMode]
            ZTest [_ZTestMode]
            Cull [_CullMode]
            // -------------------------------------
            
            HLSLPROGRAM

            // Shader Stages -----------------------
            #pragma vertex CustomForwardVertex
            #pragma fragment CustomForwardFragment
            // -------------------------------------

            // Universal Pipeline keywords ---------
            #pragma multi_compile _ _MAIN_LIGHT_SHADOWS _MAIN_LIGHT_SHADOWS_CASCADE _MAIN_LIGHT_SHADOWS_SCREEN
            #pragma multi_compile _ _ADDITIONAL_LIGHTS_VERTEX _ADDITIONAL_LIGHTS
            #pragma multi_compile_fragment _ _ADDITIONAL_LIGHT_SHADOWS
            #pragma multi_compile_fragment _ _SHADOWS_SOFT
            #pragma multi_compile_fragment _ _DBUFFER_MRT1 _DBUFFER_MRT2 _DBUFFER_MRT3
            #pragma multi_compile_fragment _ _LIGHT_LAYERS
            #pragma multi_compile_fragment _ _LIGHT_COOKIES
            // -------------------------------------

            // Unity defined keywords --------------
            #pragma multi_compile _ _RAIN
            #pragma multi_compile _ _SNOW
            #pragma multi_compile _ _CLOUD
            #pragma multi_compile _ _MASKON
            #pragma multi_compile _ _FADE
            // -------------------------------------
            
            // Material Keywords -------------------
            #pragma shader_feature_local _SHADINGMODE_BASIC _SHADINGMODE_HAIR _SHADINGMODE_EYE _SHADINGMODE_STYLIZED
            #pragma shader_feature_local _DMSMAP
            #pragma shader_feature_local _ALPHATEST
            #pragma shader_feature_local _DITHER
            #pragma shader_feature_local _SUBSURFACE
            #pragma shader_feature_local _TRIPLANAR
            #pragma shader_feature_local _TOP
            #pragma shader_feature_local _BOTTOM
            #pragma shader_feature_local _THICKNESS
            #pragma shader_feature_local _HEIGHT
            #pragma shader_feature_local _COLLISION
            #pragma shader_feature_local _EMISSION
            #pragma shader_feature_local _FRONT
            #pragma shader_feature_local _WIND
            #pragma shader_feature_local _SNOW_OFF
            #pragma shader_feature_local _RECEIVE_SHADOWS_OFF
            // -------------------------------------

            // GPU Instancing ----------------------
            #pragma multi_compile_instancing
            #pragma instancing_options renderinglayer
            // -------------------------------------

            #include "LitInput.hlsl"
            #include "ForwardPass.hlsl"
            
            ENDHLSL
        }
        
        //GBufferPass
//        Pass
//        {
//            Name "GBuffer"
//            Tags 
//            {
//                "LightMode" = "UniversalGBuffer"
//            }
//            
//            // Render State Commands ---------------
//            BlendOp [_BlendOp]
//            Blend [_SrcBlend] [_DstBlend]
//            ZWrite [_ZWriteMode]
//            ZTest [_ZTestMode]
//            Cull [_CullMode]
//            // -------------------------------------
//            
//            HLSLPROGRAM
//
//            // Shader Stages -----------------------
//            #pragma vertex CustomGBufferVertex
//            #pragma fragment CustomGBufferFragment
//            // -------------------------------------
//
//            // Universal Pipeline keywords ---------
//            #pragma multi_compile _ _MAIN_LIGHT_SHADOWS _MAIN_LIGHT_SHADOWS_CASCADE _MAIN_LIGHT_SHADOWS_SCREEN
//            // #pragma multi_compile _ _ADDITIONAL_LIGHTS_VERTEX _ADDITIONAL_LIGHTS
//            #pragma multi_compile_fragment _ _SHADOWS_SOFT
//            #pragma multi_compile_fragment _ _DBUFFER_MRT1 _DBUFFER_MRT2 _DBUFFER_MRT3
//            #pragma multi_compile_fragment _ _RENDER_PASS_ENABLED
//            // -------------------------------------
//
//            // Unity defined keywords --------------
//            #pragma multi_compile _ _RAIN
//            #pragma multi_compile _ _SNOW
//            #pragma multi_compile _ _CLOUD
//            #pragma multi_compile _ _MASKON
//            #pragma multi_compile _ _DITHER
//            #pragma multi_compile _ SHADOWS_SHADOWMASK
//            // -------------------------------------
//            
//            // Material Keywords -------------------
//            #pragma shader_feature_local _SHADINGMODE_BASIC _SHADINGMODE_HAIR _SHADINGMODE_SUBSURFACE _SHADINGMODE_STYLIZED _SHADINGMODE_EYE _SHADINGMODE_SKIN
//            #pragma shader_feature_local _DMSMAP
//            #pragma shader_feature_local _ALPHATEST
//            #pragma shader_feature_local _TRIPLANAR
//            #pragma shader_feature_local _TOP
//            #pragma shader_feature_local _BOTTOM
//            #pragma shader_feature_local _THICKNESS
//            #pragma shader_feature_local _HEIGHT
//            #pragma shader_feature_local _EMISSION
//            #pragma shader_feature_local _WIND
//            #pragma shader_feature_local _CLOUD
//            #pragma shader_feature_local _RECEIVE_SHADOWS_OFF
//            //#pragma shader_feature_local _STYLIZED
//            // -------------------------------------
//
//            // GPU Instancing ----------------------
//            #pragma multi_compile_instancing
//            #pragma instancing_options renderinglayer
//            // -------------------------------------
//
//            #include "LitInput.hlsl"
//            #include "GBufferPass.hlsl"
//
//            ENDHLSL
//        }    
        
        Pass
        {
            Name "ShadowCaster"
            Tags
            {
                "LightMode" = "ShadowCaster"
            }
            
            // Render State Commands ---------------
            ZWrite On
            ZTest LEqual
            ColorMask 0
            Cull [_CullMode]
            // -------------------------------------
            
            HLSLPROGRAM

            // Shader Stages -----------------------
            #pragma vertex ShadowPassVertex
            #pragma fragment ShadowPassFragment
            // -------------------------------------

            // Universal Pipeline keywords ---------

            // -------------------------------------

            // Unity defined keywords --------------
            // #pragma multi_compile_vertex _ _CASTING_PUNCTUAL_LIGHT_SHADOW
            // #pragma multi_compile_fragment _ LOD_FADE_CROSSFADE
            #pragma multi_compile _ _FADE
            // -------------------------------------
            
            // Material Keywords -------------------
            #pragma shader_feature_local _THICKNESS
            #pragma shader_feature_local _HEIGHT
            #pragma shader_feature_local _WIND
            #pragma shader_feature_local _COLLISION
            #pragma shader_feature_local _ALPHATEST
            //#pragma shader_feature_local _DITHER
            // -------------------------------------

            // GPU Instancing ----------------------
            #pragma multi_compile_instancing
            // -------------------------------------

            #include "LitInput.hlsl"
            #include "ShadowCasterPass.hlsl"

            ENDHLSL
        }
        
        Pass
        {
            Name "DepthOnly"
            Tags
            {
                "LightMode" = "DepthOnly"
            }

            // Render State Commands ---------------
            ColorMask R
            BlendOp [_BlendOp]
            Blend [_SrcBlend] [_DstBlend]
            ZWrite [_ZWriteMode]
            ZTest [_ZTestMode]
            Cull [_CullMode]
            // -------------------------------------

            HLSLPROGRAM

            // Shader Stages -----------------------
            #pragma vertex DepthOnlyVertex
            #pragma fragment DepthOnlyFragment

            // Universal Pipeline keywords ---------
            #pragma multi_compile _ _FORWARD_PLUS
            // -------------------------------------
            
            // Unity defined keywords --------------
            #pragma multi_compile _ _FADE
            // -------------------------------------
            
            // Material Keywords -------------------
            #pragma shader_feature_local _ALPHATEST
            //#pragma shader_feature_local _DITHER
            #pragma shader_feature_local _THICKNESS
            #pragma shader_feature_local _HEIGHT
            #pragma shader_feature_local _WIND
            #pragma shader_feature_local _COLLISION
            // -------------------------------------

            // GPU Instancing ----------------------
            #pragma multi_compile_instancing
            // -------------------------------------

            #include "LitInput.hlsl"
            #include "DepthOnlyPass.hlsl"
            
            ENDHLSL
        }

//        Pass
//        {
//            Name "DepthNormals"
//            Tags
//            {
//                "LightMode" = "DepthNormals"
//            }
//
//            // -------------------------------------
//            // Render State Commands
//            ZWrite On
//            Cull [_CullMode]
//
//            HLSLPROGRAM
//
//            // -------------------------------------
//            // Shader Stages
//            #pragma vertex DepthNormalsVertex
//            #pragma fragment DepthNormalsFragment
//
//            // -------------------------------------
//            // Material Keywords
//            #pragma multi_compile _ _FADE
//            // -------------------------------------
//            
//            // Unity defined keywords
//            #pragma shader_feature_local _ALPHATEST
//            //#pragma shader_feature_local _DITHER
//            #pragma shader_feature_local _THICKNESS
//            #pragma shader_feature_local _HEIGHT
//            #pragma shader_feature_local _WIND
//            #pragma shader_feature_local _COLLISION
//            // -------------------------------------
//            
//            // Universal Pipeline keywords
//
//            //--------------------------------------
//            // GPU Instancing
//            #pragma multi_compile_instancing
//
//            // -------------------------------------
//            // Includes
//            #include "LitInput.hlsl"
//            #include "DepthNormalsPass.hlsl"
//            ENDHLSL
//        }
    }
    
    CustomEditor "EBGame.SimpleShaderGUI"
}