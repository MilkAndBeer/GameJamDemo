Shader "Hidden/Custom/LayeredFog"
{
    ...
    SubShader
    {
        ...
        Pass
        {
            ...
            float4 Fog(float2 uv, float4 screen_pos, float3 positionWS)
            {
                ...
                const float3 worldPos = screen_pos.xyz * depthEye + _WorldSpaceCameraPos;
                ...
                float3 uvw = positionWS * (1.0 / _NoiseScale) + (-_NoiseSpeed * _TimeParameters.x * 2.0);

                float noise = SimpleNoise3D(uvw);
                noise = noise * 0.5 + 0.5;   // scale and offset(0.5, 0.5)

                // noise distance mask
                half nosieDistanceMask = saturate((distance(positionWS , _WorldSpaceCameraPos) - _NoiseDistanceEnd) / (0.0 - _NoiseDistanceEnd));
                noise = lerp(1, noise, nosieDistanceMask * _NoiseIntensity);

                float4 final = lerp(original, fog, fog.a * noise);  // optimization: float3
                final.a = original.a;
                // final.rgb = uvw;
            ...
            Varyings vert(Attributes input)
            {
                ...
                output.screen_pos = ComputeScreenPos(output.positionCS);
            ...
            half4 frag(Varyings input): SV_Target
            {
                ...
                float2 positionNDC = input.screen_pos.xy / input.positionNDC.w;
                ...
                float4 c = Fog(input.uv, input.screen_pos, worldPos);
