...
half4 LitPassFragment(Varyings input) : SV_Target
{
    ...
    half sunBloomMask = max(0, sunBloomSize) * pow(saturate(positionWS.y + 1), 10);
    half3 sunBloomColor = lerp(_SunBloomColor, skyColor, _SunDirWS.y) * sunBloomMask;
    
    half3 sun = sunColor + sunBloomColor;
 
    //Moon
    half moonIntersect = sphIntersect(positionWS, _SunDirWS, _MoonSize * 0.1);
    half moonDist = distance(positionWS, _SunDirWS);
    half moonSize = 1 - saturate(moonDist / (_MoonSize * 0.1));
    half moonMask = smoothstep(0, _MoonFalloff, moonSize) * halfMask;
    
    half3 moonNormal = normalize(_SunDirWS - positionWS * moonIntersect);
    ...
    half crescentMoonDist = distance(half3(positionWS.x + _MoonBlock * _MoonSize, positionWS.y, positionWS.z), _SunDirWS);
    ...
    half moonBloomSize = smoothstep(1 - _MoonBloomRange, 1 - _MoonBloomRange + _MoonBloomFalloff, saturate(dot(_SunDirWS, positionWS))) * _MoonBloomIntensity;
    half moonBloomMask = moonBloomSize * pow(saturate(positionWS.y + 1), 10);
    half3 moonBloomColor = lerp(_MoonBloomColor, skyColor, _SunDirWS.y) * moonBloomMask;
