...
// #endif
    ...
    CustomData customData = InitializecustomData(input.uv, positionWS, input.positionOS, input.positionSS, shadowCoord,
                                                 input.normalWS.xyz, input.tangentWS.xyz, input.bitangentWS.xyz, input.normalOS, input.uv.xy);

    float3 normalWS = customData.normalWS;w
    outNormalWS = float4(NormalizeNormalPerPixel(normalWS), 0.0);
}