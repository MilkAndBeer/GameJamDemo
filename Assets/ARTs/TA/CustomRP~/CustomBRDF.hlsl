...
#include "Assets/ARTs/TA/Resources/CustomRP/CustomBSDF.hlsl"
...
//Stylized SpecularTerm
half DirectBRDFSpecularStylized(BRDFData brdfData, SurfaceData surfaceData, half3 normalWS, half3 lightDirectionWS, half3 viewDirectionWS)
{
    float3 lightDirectionWSFloat3 = float3(lightDirectionWS);
    float3 halfDir = SafeNormalize(lightDirectionWSFloat3 + float3(viewDirectionWS));
    ...
    specularTerm = LinearStep(surfaceData.specThreshold - surfaceData.specSmooth, surfaceData.specThreshold + surfaceData.specSmooth, specularTerm) *
        max(0, surfaceData.specIntensity);

    return specularTerm;
}

//Hair SpecularTerm
half3 DirectBRDFSpecularHair(BRDFData brdfData, SurfaceData surfaceData, half3 normalWS, half3 lightDirectionWS, half3 viewDirectionWS)
{
    float3 lightDirectionWSFloat3 = float3(lightDirectionWS);
    float3 halfDir = SafeNormalize(lightDirectionWSFloat3 + float3(viewDirectionWS));
    ...
    half LoH2 = LoH * LoH;
    half3 specularTerm = brdfData.roughness2 / ((d * d) * max(0.1h, LoH2) * brdfData.normalizationTerm);
    ...
    //specularTerm = D_KajiyaKay(surfaceData.tangentWS, halfDir, surfaceData.hairSpecExponent);
    specularTerm = StrandSpecular(surfaceData.tangentWS, viewDirectionWS, lightDirectionWSFloat3, surfaceData.hairSpecExponent) *
         saturate(lerp(0.25, 1.0, dot(normalWS, lightDirectionWSFloat3))) * surfaceData.hairSpecMask;
    
    specularTerm = LinearStep(surfaceData.specThreshold - surfaceData.specSmooth, surfaceData.specThreshold + surfaceData.specSmooth, specularTerm) *
                                max(0, surfaceData.specIntensity) * surfaceData.hairSpecMask;
