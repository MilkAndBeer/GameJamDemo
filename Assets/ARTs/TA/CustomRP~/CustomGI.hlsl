...
#include "Assets/ARTs/TA/Resources/CustomRP/CustomIBL.hlsl"
#include "Assets/ARTs/TA/Resources/CustomRP/CustomSurfaceData.hlsl"
...
half3 StylizedGlobalIllumination(BRDFData brdfData, SurfaceData surfaceData, half3 bakedGI, half occlusion,
                                float3 positionWS, half3 normalWS, half3 viewDirectionWS, half3 lightDirWS)
{
    half3 reflectVector = reflect(-viewDirectionWS, normalWS);
    half NoV = saturate(dot(normalWS, viewDirectionWS));
    half NoL = saturate(dot(normalWS, lightDirWS));
    half fresnelTerm = Pow4(1.0 - NoV) * surfaceData.fresIntensity;

    half halfLambert = LinearStep(surfaceData.shadowThreshold - surfaceData.shadowSmooth, surfaceData.shadowThreshold + surfaceData.shadowSmooth,
                                    dot(normalWS, lightDirWS) * 0.5 + 0.5);
    fresnelTerm = LinearStep(surfaceData.fresThreshold - surfaceData.fresSmooth, surfaceData.fresThreshold + surfaceData.fresSmooth, fresnelTerm)
                    * max(0, surfaceData.fresIntensity) * (1 - NoL) * halfLambert;

    //float surfaceReduction = 1.0 / (brdfData.roughness2 + 1.0);
    //half3 indirectSpecularTerm = surfaceReduction * lerp(brdfData.specular, brdfData.grazingTerm * surfaceData.fresColor, fresnelTerm);
    //half3 ambient = max(half3(0, 0, 0), SampleSH9(surfaceData.customSH, normalWS)) * surfaceData.occlusion * surfaceData.exposure;
    half3 indirectDiffuse = brdfData.diffuse * surfaceData.exposure * bakedGI;
    half3 indirectSpecular = GlossyEnvironmentReflection(reflectVector, positionWS, brdfData.perceptualRoughness, 1.0h) * surfaceData.reflectExposure;
    //half3 indirectSpecular = GlossyEnvironmentReflectionStylized(reflectVector, positionWS, brdfData.perceptualRoughness, surfaceData.occlusion, surfaceData.reflectExposure) * indirectSpecularTerm;
